...
router.get('/professor', async (req: Request<Professor>, res: Response, next: NextFunction) => {
  try {
    const professores: Professor[] = await new ProfessorController().listar({tipo:Number(req.query.tipo)});
